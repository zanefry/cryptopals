...
char** bytes_to_binary(char* bytes, int num_bytes);
...
int main() {
  ...
  num_bytes = strlen(hex) % 2 == 0 ? strlen(hex) / 2 : (strlen(hex) + 1) / 2;
  binary = bytes_to_binary(bytes, num_bytes);
...
/////////////// binary string ////////////////
// Takes a byte and returns a string containing its binary representation
char* byte_to_binary(char byte) {
  char* binary = calloc(9, sizeof(char));

  int p = 1;
  for(int i = 0; i < 8; i++) {
    binary[7 - i] = (byte & p) == p ? '1' : '0';
    p *= 2;
  }
}

// Takes a string of bytes and the length of the string
// (because it may contain 0 bytes) and returns a buffer
// containing the binary representations of the bytes
char** bytes_to_binary(char* bytes, int num_bytes) {
  char** binary = malloc(sizeof(char*) * num_bytes);

  for(int i = 0; i < num_bytes; i++)
    binary[i] = byte_to_binary(bytes[i]);
